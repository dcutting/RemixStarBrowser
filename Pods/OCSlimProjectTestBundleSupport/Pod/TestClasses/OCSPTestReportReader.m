...
- (NSData *) read {
    ...
    NSString *path = @"/tmp/Fitnesse-Test-Report.xml";//[bundle pathForResource:fitnesseTestReportResourceName ofType:@"xml"];
    
    if (path == nil ) {
       
        NSLog(@"[OCSP] ERROR: Could not read data : The file \"%@\" in bundle \"%@\" couldn’t be opened because there is no such file: Check your 'OCSlimProject' Acceptance Test target is being built. 1) Try running 'pod update' to resolve this issue. 2) Manually run Acceptance Tests using '$ ./LaunchFitnesse' located at your projects root directory and investigate any issues.", path, bundle, nil);
